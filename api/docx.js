...
function buildParagraph(text, { bold = false, size = null, color = null, bullet = false } = {}) {
  const rPr = [
  ...
  const numPr = bullet ? '<w:numPr><w:ilvl w:val="0"/><w:numId w:val="0"/></w:numPr>' : '';
...
function buildDocumentXml(parsed) {
  ...
  paragraphs.push(buildParagraph(parsed.name, { bold: true, size: 36 }));
  paragraphs.push(buildParagraph(parsed.contact, { size: 20, color: '5B6270' }));

  parsed.sections.forEach(sec => {
    paragraphs.push(buildParagraph(sec.header, { bold: true, size: 24, color: '57623F' }));
    sec.paragraphs.forEach(p => paragraphs.push(buildParagraph(p)));
    sec.bullets.forEach(b => paragraphs.push(buildParagraph(`•  ${b}`)));
  });
...
<w:body>
${paragraphs.join('\n')}
<w:sectPr><w:bidi/><w:pgSz w:w="11906" w:h="16838"/><w:pgMar w:top="1417" w:right="1417" w:bottom="1417" w:left="1417" w:header="708" w:footer="708" w:gutter="0"/></w:sectPr>
</w:body>
</w:document>`;
}
...
<w:docDefaults>
<w:rPrDefault><w:rPr><w:rFonts w:ascii="Arial" w:hAnsi="Arial" w:cs="Arial" w:eastAsia="Arial"/><w:rtl/><w:lang w:val="he-IL" w:eastAsia="he-IL" w:bidi="he-IL"/></w:rPr></w:rPrDefault>
<w:pPrDefault><w:pPr><w:bidi/><w:jc w:val="start"/></w:pPr></w:pPrDefault>
</w:docDefaults>
